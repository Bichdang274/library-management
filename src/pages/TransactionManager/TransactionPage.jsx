...
const TransactionPage = () => {
  // State để chuyển đổi giữa 2 tab: 'borrow' (Mượn) và 'return' (Trả)
  ...
  // Tải dữ liệu khi chuyển sang tab 'Trả Sách'
  useEffect(() => {
    if (activeTab === 'return') {
      fetchActiveLoans();
    }
  ...
  const handleBorrow = async (e) => {
    ...
    try {
      ...
      setFormData({ reader_id: '', book_id: '', due_date: '' }); // Reset form
    } catch (error) {
      setMessage({ 
  ...
  const handleReturn = async (borrow_id, book_id) => {
    ...
    try {
      ...
      fetchActiveLoans(); // Load lại bảng sau khi trả
